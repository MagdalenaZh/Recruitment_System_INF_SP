...
namespace AppilicationProcesserAPI.Controllers
{
    ...
    public class AplicationProcessingController : ControllerBase
    {
        ...
        {
            if (!await _authorizationScopeService.CanSubmitApplicationAsync(User, applicationData.DepartmentId, cancellationToken).ConfigureAwait(false))
            {
                return Forbid();
            }
        ...
        {
            if (!await _authorizationScopeService.CanBookInterviewSlotAsync(User, applicationId, cancellationToken).ConfigureAwait(false))
            {
                return Forbid();
            }
        ...
        {
            if (!await _authorizationScopeService.CanConcludeApplicationAsync(User, applicationId, cancellationToken).ConfigureAwait(false))
            {
                return Forbid();
            }
